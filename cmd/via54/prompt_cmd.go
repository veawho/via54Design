...
func promptHelp() {
	...
	fmt.Println("  i2i                 v2.3 双语 i2i 提示词引擎 (飞书入口)")
	fmt.Println("    --scene \"...\"  --platform jimeng  [--ref ref.jpg] [--ref-desc \"...\"]")
	fmt.Println("    [--max-chars 1500] [--overrides '{\"subject\":\"红发\"}']")
}
...
// ─── I2I (image-to-image bilingual engine) ────────────────────────────────
//
// v2.3: Feishu/i2i 工作流入口
//   via54 prompt i2i --scene "温馨家庭客厅" --platform jimeng \
//     --ref /tmp/ref.jpg --ref-desc "modern living room..." --max-chars 1500
//
// 输出 JSON: I2IResult JSON form (FinalEN, FormattedZH, Sections, ...).
// 飞书 bot / inbox_watcher 消费这个 JSON。
func cmdPromptI2I() {
	fs := flag.NewFlagSet("i2i", flag.ExitOnError)
	scene := fs.String("scene", "", "用户场景/需求 (中文/英文均可)")
	platform := fs.String("platform", "jimeng", "目标平台")
	ref := fs.String("ref", "", "参考图片路径")
	refDesc := fs.String("ref-desc", "", "参考图的视觉描述 (caller-extracted via LLaVA / mmx)")
	maxChars := fs.Int("max-chars", 0, "提示词字符限额 (0 = 平台默认)")
	overrides := fs.String("overrides", "", "字段覆盖 JSON, 形如 '{\"subject\":\"红发蓝裙\"}'")
	output := fs.String("output", "", "输出 JSON 文件 (默认 stdout)")
	format := fs.String("format", "json", "输出格式: json/markdown (markdown 只输出 FormattedZH)")
	baseDirFlag := fs.String("base-dir", "", "via54Design 仓库根目录 (默认走 FindBaseDir)")
	fs.Parse(os.Args[3:])
	...
	if !isValidPlatform(*platform) {
		fmt.Fprintf(os.Stderr, "❌ 未知平台: %q\n可用: %s\n", *platform, strings.Join(listPlatforms(), ", "))
		os.Exit(1)
	}

	var ovrMap map[string]string
	if *overrides != "" {
		if err := json.Unmarshal([]byte(*overrides), &ovrMap); err != nil {
			fmt.Fprintf(os.Stderr, "❌ --overrides JSON 解析失败: %v\n", err)
			os.Exit(1)
		}
	}
	...
	result, err := prompt.GenerateI2I(prompt.I2IRequest{
		Scene:          *scene,
		Platform:       *platform,
		RefImage:       *ref,
	...
	if *format == "markdown" {
		// 用户侧 markdown 视图 (中文格式化, 含英文提示词)
		if *output != "" {
			os.WriteFile(*output, []byte(result.FormattedZH), 0644)
			fmt.Printf("✅ 中文格式化提示词: %s\n", *output)
		} else {
			fmt.Print(result.FormattedZH)
		}
		return
	}
	...
	if *output != "" {
		os.WriteFile(*output, js, 0644)
		fmt.Printf("✅ I2I JSON: %s (chars=%d/%d)\n", *output, result.FinalChars, result.MaxChars)
	} else {
		fmt.Print(string(js))
	}
}

// cmdPromptFillPlan outputs a FillPlan JSON for the caller to hand to the LLM.
// Use case: hermes expands unfilled fields, then re-runs i2i with the result
// as --overrides.
//
//   via54 prompt i2i-fill --input /tmp/i2i-v1.json --ref-desc "..."
//   # outputs FillPlan JSON to stdout
func cmdPromptFillPlan() {
	fs := flag.NewFlagSet("i2i-fill", flag.ExitOnError)
	input := fs.String("input", "", "I2I JSON 文件 (from `via54 prompt i2i --output ...`)")
	refDesc := fs.String("ref-desc", "", "参考图描述 (用于 ref-anchor 注入)")
	bd := fs.String("base-dir", baseDir(), "via54Design 仓库根目录")
	fs.Parse(os.Args[3:])

	if *input == "" {
		fmt.Fprintln(os.Stderr, "请指定 --input <i2i.json>")
		os.Exit(1)
	}
	...
	var result prompt.I2IResult
	if err := json.Unmarshal(data, &result); err != nil {
		fmt.Fprintf(os.Stderr, "JSON 解析失败: %v\n", err)
		os.Exit(1)
	}
	_ = bd
	plan := prompt.PlanFill(&result, *refDesc)
	js, _ := json.MarshalIndent(plan, "", "  ")
	fmt.Print(string(js))
}

// TranslateViaHermes is a thin wrapper that shells out to `hermes -z` for
// free-text field translation. It is intentionally a separate function so
// tests can stub the hermes call.
//
// The Go engine itself never imports an LLM client — translation is always
// external. This function lives in cmd/ (not internal/prompt) because
// shelling out to a CLI is a deployment concern, not a core prompt-engine
// concern.
//
// mode: "en_zh" or "zh_en"
func TranslateViaHermes(text, refDesc, platform, mode string) (string, error) {
	if text == "" {
		return "", nil
	}
	// Build a self-contained prompt for hermes
	var sysPrompt string
	if mode == "en_zh" {
		sysPrompt = "你是专业的中英翻译。把英文翻成格式化的中文,保留所有摄影/构图/技术术语原文。\n"
	} else {
		sysPrompt = "你是专业的英中翻译。把中文需求扩展并翻译成专业的英文生图提示词,保留所有专业术语。\n"
	}
	if refDesc != "" {
		sysPrompt += "参考图描述 (翻译时必须保留这些视觉特征): " + refDesc + "\n"
	}
	sysPrompt += "目标平台: " + platform + "\n"
	sysPrompt += "原文:\n" + text + "\n\n只输出翻译结果,不要解释。"

	// 找 hermes CLI
	hermesBin := os.Getenv("VIA54_HERMES_BIN")
	if hermesBin == "" {
		hermesBin = "/Users/david/.hermes/hermes-agent/venv/bin/hermes"
	}
	cmd := exec.Command(hermesBin, "-z", sysPrompt)
	cmd.Env = append(os.Environ(),
		"PATH=/Users/david/.hermes/hermes-agent/venv/bin:/Users/david/.local/bin:/usr/bin:/bin",
	)
	out, err := cmd.CombinedOutput()
	if err != nil {
		return text, fmt.Errorf("hermes -z failed: %w (%s)", err, strings.TrimSpace(string(out))[:200])
	}
