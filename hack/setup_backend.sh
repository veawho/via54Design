...
echo "╔═════════════════════════════════════════════════════════╗"
echo "║  via54Design × 生图后端部署脚本 (Forge Classic + ComfyUI) ║"
echo "╚═════════════════════════════════════════════════════════╝"
echo ""
...
MODE="${1:---auto}"
case "$MODE" in
    --forge)  INSTALL_MODE="forge" ;;
    --comfy)  INSTALL_MODE="gpu" ;;
...
case "$INSTALL_MODE" in
    ...
    forge)
        echo "  安装 Forge Classic (Neo分支)..."
        echo ""
        echo "  步骤:"
        echo "    git clone https://github.com/Haoming02/sd-webui-forge-classic.git"
        echo "    cd sd-webui-forge-classic"
        echo "    git checkout neo"
        echo "    python -m venv venv && source venv/bin/activate"
        echo "    pip install -r requirements.txt"
        echo "    python launch.py --listen 0.0.0.0 --port 7860"
        echo ""
        echo "  ⚡ 推荐加速:"
        echo "    pip install sageattention flash-attn"
        echo "    python launch.py --listen 0.0.0.0 --port 7860 --sage-attention"
        echo ""
        echo "  🔧 via54 集成:"
        echo "    via54 forge --workflow sdxl_txt2img --prompt "...""
        echo "    # 自动提交到 http://localhost:7860"
        ;;
...
# ═══ Step 7: 降级方案 ═══
echo "═══ 7/7: 降级方案 (无需任何后端) ═══"
echo ""
echo "  via54Design 的 Prompt 引擎完全独立于生图后端："
echo ""
echo "  ┌──────────────────────────────────────────────────────────┐"
echo "  │ 方案 A: 直接提交到云平台                                │"
