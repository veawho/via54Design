...
func AssessImage(imagePath string, promptText string) *QualityReport {
	...
	if info, err := os.Stat(imagePath); err == nil {
		...
		if mb < 0.1 { r.ClarityScore = 0.3; r.Issues = append(r.Issues, "文件<100KB, 可能低质") } else if mb > 10 { r.ClarityScore = 0.9 } else { r.ClarityScore = 0.6 + (mb/10)*0.3 }
	}
	if promptText != "" {
		w := len(strings.Fields(promptText))
		if w > 50 { r.PromptMatch = 0.8 } else if w > 20 { r.PromptMatch = 0.6 } else { r.PromptMatch = 0.4 }
	}
	r.CompositionScore = 0.7; r.ColorScore = 0.7
...
func (r *QualityReport) String() string {
	...
	for _, iss := range r.Issues { s += fmt.Sprintf("  ⚠️ %s\n", iss) }
	return s
}
