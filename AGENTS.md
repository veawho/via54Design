...
- **v2.3 i2i 引擎 (2026-06-12 巫师叔叔)**:
  - `--ref` 路径必须配合 `--ref-desc`(caller 负责从图片提取视觉描述, 推荐 LLaVA / mmx `image interrogate`)
  - `subject`/`secondary`/`environment` 三个 ref-locked 字段语义不同: 身份 vs 风格一致 vs 身份+场景, 不要重复
  - 限额算法 `PlanFill(result, refDesc)` 在 `cmdPromptFillPlan` 暴露, 输出 JSON 给 LLM 扩写
  - **安装到 `/usr/local/bin/via54` 会被 macOS spctl 拒绝** (Gapped 0/root ownership), 必须装到 `~/.local/bin/via54` 跳过 quarantine
  - macOS Sequoia `com.apple.provenance` xattr 让 launchd 子进程拒绝读取 — 飞书 bot 路径直接调用户 bin, 不要走 launchd plist
...
- **版本**: v0.5.0 (v2.3 i2i engine)
- **Go二进制**: ~18MB, 单文件, 零外部依赖
- **测试套件**: `test_20_rounds.py` (Python 3.6+ stdlib only) + Go 单元测试 (`internal/prompt/i2i_test.go` v2.3 新增 11 个)
...
- **v2.3 新增能力**:
  - 双语 i2i 提示词生成 (中文格式化 markdown + 英文最终 prompt)
  - Ref-lock 三段语义: subject 锁身份 / secondary 锁风格一致 / environment 锁身份+场景
  - 16 平台限额最优填充 (jimeng 1500 / midjourney 4500 / gemini 8000 / sd 2000 等)
  - 飞书 `/i2i-prompt` 状态机: 首次生成 → 用户改 → 确认返回完整英文
- **稳定性**: Go 单元测试 31/31 PASS, Python 端 e2e 验证全平台输出
- **SPDX覆盖率**: 60+ Go 源文件 (100%)
...
./via54.exe prompt i2i --scene "..." --platform jimeng --ref ref.jpg --ref-desc "..."  # v2.3 双语 i2i
./via54.exe prompt i2i-fill --input i2i.json --ref-desc "..."  # v2.3 限额最优 fill plan
python test_20_rounds.py             # 20 轮端到端测试
go test ./internal/prompt/            # Go 单元测试 (31 个, v2.3 新增 11 个)
```
